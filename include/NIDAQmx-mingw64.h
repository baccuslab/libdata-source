/*! \file NIDAQmx-mingw64.h
 * A copy of the National Instruments DAQmx header used with
 * MINGW64.
 *
 * The DAQmx C library is designed to work only on Windows, which means
 * certain datatypes in this header are defined in terms of Windows-
 * specific types. But on MINGW these are no longer defined. This 
 * files is nearly an exact copy of the standard NIDAQmx.h file, with
 * those typedefs modified to compile in MINGW.
 */
...
#if defined(__linux__) || defined(__APPLE__) || defined(__MINGW64__)
